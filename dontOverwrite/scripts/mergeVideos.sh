...
do
  ...
  let file_num=$(cd $TMPDIR && ls -l | grep -v ^d | wc -l)-1

  if [ $file_num -gt 5 ]
  then
    run=0
     #Generate textfile with filenames in tmp then delete all files in tmp
    cd /home/pi/qmapboxglapp-tmpsantos-cheapruler/frontCamRecordings/tmp && printf "file '/home/pi/qmapboxglapp-tmpsantos-cheapruler/frontCamRecordings/tmp/%s'\n" * > /home/pi/qmapboxglapp-tmpsantos-cheapruler/frontCamRecordings/filesToMerge && printf "'%s'\n" * > /home/pi/qmapboxglapp-tmpsantos-cheapruler/frontCamRecordings/filesToDelete && cd ..
     	
    ffmpeg -f concat -i filesToMerge -c copy -fflags +genpts concat_$DATETIME.mp4 &
    let pid=$!
    wait $pid
